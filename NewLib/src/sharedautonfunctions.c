...
bool platform_climb(int latch, int roomba){
  int draw = (motor_get_current_draw(right_mg[1]) + motor_get_current_draw(left_mg[1]))/2;
  int velocity = ( motor_get_actual_velocity(right_mg[1]) +  motor_get_actual_velocity(left_mg[1])) / 2;
  printf("d-%d v-%d\n",draw,velocity );
  latch_limit = max((abs(draw)/abs(velocity)), latch_limit);
   printf("latch limit %d value %d\n",latch_limit,(abs(velocity)));
   return(latch_limit>latch && abs(velocity) >roomba );
...
void go_platform(void)
{
  ...
  while(!platform_climb(4000,20)){
    motor_move(left_mg[0], -200);
