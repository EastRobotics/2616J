...
//  printf("%d\r\n",adi_analog_read_calibrated('F'));
    ...
    descorerBackward = controller_get_digital(E_CONTROLLER_MASTER, E_CONTROLLER_DIGITAL_A);
      ...
      // sets the speed of the motors and how they should function
		...
		// else if (descorerFward == 1)
		// 	motor_move(MOTOR_DESCORER, descorerBackward * -127);
		// else
		// 	motor_move(MOTOR_DESCORER, 0);in

		if (intakeForward == 1)
    {
      motor_move(MOTOR_INTAKE_RIGHT, intakeForward * 127);
      motor_move(MOTOR_INTAKE_LEFT, intakeForward * -127);
    }
    else if (intakeForward == 0)
    {
      motor_move(MOTOR_INTAKE_RIGHT, intakeForward * 0);
      motor_move(MOTOR_INTAKE_LEFT, intakeForward * 0);
    }
  ...
  else if (intakeBackward == 0)
  {
    motor_move(MOTOR_INTAKE_RIGHT, intakeForward * 0);
    motor_move(MOTOR_INTAKE_LEFT, intakeForward * 0);
  }

// else if (intakeForward == 1)
// {
//   motor_move(MOTOR_INTAKE_RIGHT, intakeForward * -127);
//   motor_move(MOTOR_INTAKE_LEFT, intakeForward * 127);
// }
